-- =====================================================================
-- 1. MEMBERSHIP ID GENERATOR (NACOS ID: NA-CSC-XXX)
-- =====================================================================
...
BEGIN
  -- Check for specific reserved executive positions (only if not already taken)
  IF pos = 'president' AND NOT EXISTS (SELECT 1 FROM public.profiles WHERE membership_id = prefix || '001') THEN
    RETURN prefix || '001';
  ELSIF (pos LIKE '%director of software%' OR pos LIKE '%software director%') AND NOT EXISTS (SELECT 1 FROM public.profiles WHERE membership_id = prefix || '002') THEN
    RETURN prefix || '002';
  ELSIF (pos LIKE '%stakeholder%chairman%' OR pos LIKE '%chairman%stakeholder%') AND NOT EXISTS (SELECT 1 FROM public.profiles WHERE membership_id = prefix || '003') THEN
    RETURN prefix || '003';
  END IF;
  ...
  SELECT COALESCE(
    MAX(
      NULLIF(
        REGEXP_REPLACE(membership_id, '^NA-CSC-', ''), 
        ''
      )::INTEGER
    ), 
    3
  ...
  WHERE membership_id LIKE 'NA-CSC-%'
    AND membership_id NOT IN ('NA-CSC-001', 'NA-CSC-002', 'NA-CSC-003');
...
-- Create RPC function to allow existing users to assign their own ID
...
BEGIN
  ...
  SELECT position INTO user_pos
  FROM public.profiles
  WHERE user_id = auth.uid();

  -- Generate ID
  new_id := public.generate_nacos_id(user_pos);

  -- Update the profile if membership_id is null
  UPDATE public.profiles
  SET membership_id = new_id
  WHERE user_id = auth.uid() AND membership_id IS NULL;

  -- Return the ID (or their existing ID if already set)
  SELECT membership_id INTO new_id
  FROM public.profiles
  WHERE user_id = auth.uid();
...
-- One-time update: Assign membership IDs to existing users who don't have one
-- We do it row-by-row in a loop to ensure sequential assignment works correctly
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN SELECT id, position FROM public.profiles WHERE membership_id IS NULL LOOP
    UPDATE public.profiles
    SET membership_id = public.generate_nacos_id(r.position)
    WHERE id = r.id;
  END LOOP;
END $$;
